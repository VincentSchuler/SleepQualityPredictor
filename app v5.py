...
st.markdown(
    ...
    .block-container {padding-top: 0.8rem; padding-bottom: 1.6rem; max-width: 1420px;}
    ...
    .small-note {color:#9fb2c8; font-size:0.95rem; margin-top: 0.15rem; margin-bottom: 0.8rem;}
    ...
    .factor-meta {display:flex; flex-wrap:wrap; gap:0.45rem; margin-top:0.58rem;}
    .hint-chip {
        display:inline-flex;
        align-items:center;
    ...
    .hint-chip-risk {background: rgba(255,255,255,0.10);}
    .hint-chip-good {background: rgba(255,255,255,0.10);}
...
TEXT = {
    ...
    "EN": {
        ...
        "quality_band": [
            ...
            (75, 101, "Healthy quality"),
        ],
        "high_value": "High value",
        "low_value": "Low value",
        "protective": "Favorable value",
        "present": "Present",
        "absent": "Absent",
    },
}
        ...
        go.Indicator(
            ...
            gauge={
                ...
                "steps": [
                    ...
                    {"range": [50, 75], "color": "rgba(212, 170, 55, 0.18)"},
                    {"range": [75, 100], "color": "rgba(46, 204, 113, 0.20)"},
                ],
            },
        )
    )
    fig.update_layout(height=350, margin=dict(l=20, r=20, t=0, b=0), paper_bgcolor="rgba(0,0,0,0)")
    return fig
...
def body_label_from_bmi(value: float, lang: str) -> str:
    ...
    best_label, best_bmi = min(options, key=lambda item: abs(item[1] - float(value)))
    if lang == "FR":
        return f"{best_label} (IMC estimé {float(value):.1f})"
    return f"{best_label} (est. BMI {float(value):.1f})"
...
def explain_hint(feature: str, value, impact: float, lang: str) -> str:
    v = value
    high_label = tr(lang, "high_value")
    low_label = tr(lang, "low_value")
    protective = tr(lang, "protective")
    ...
    if feature == "sleep_duration_hrs":
        if float(v) < 6.5:
            return low_label
        return protective if impact < 0 else high_label
    if feature == "sleep_latency_mins":
        return high_label if int(v) >= 30 else low_label
    if feature == "wake_episodes_per_night":
        return high_label if int(v) >= 2 else low_label
    if feature == "stress_score":
        return high_label if float(v) >= 6 else low_label
    if feature == "bmi":
        if float(v) >= 27:
            return high_label
        if float(v) <= 18.5:
    ...
    if feature == "work_hours_that_day":
        return high_label if float(v) >= 9 else low_label
    if feature == "alcohol_units_before_bed":
        return high_label if float(v) > 0 else low_label
    if feature == "nb_cafe_before_bed":
        return high_label if int(v) >= 2 else low_label
    if feature in {"Anxiety", "Depression"}:
        return present if int(v) == 1 else absent
    if feature == "time_screen_before_sleep":
        value_str = str(v).lower()
        return high_label if ("60" in value_str or "2h+" in value_str or "2h" in value_str) else low_label
    if feature == "age":
        return high_label if int(v) >= 60 else protective
    return protective if impact < 0 else high_label
...
def render_factor_cards(title: str, factors: list, lang: str, positive: bool) -> None:
    ...
    chip_class = "hint-chip-risk" if positive else "hint-chip-good"
        ...
        st.markdown(
            ...
            <div class='factor-card {card_class}'>
                ...
                <div class='factor-meta'>
                    <span class='hint-chip {chip_class}'>{hint}</span>
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with st.container(border=False):
        ...
        with row3[1]:
            nb_cafe_before_bed = st.slider(
                tr(lang, "coffee"), 0, 3, int(FORM_DEFAULTS["nb_cafe_before_bed"]), 1,
                format="%d"
            )
        with row3[2]:
...
def main() -> None:
    title_col, lang_col = st.columns([0.78, 0.22])
    with title_col:
        st.markdown(f"<div class='page-title'>{tr('FR', 'title')}</div>", unsafe_allow_html=True)
    with lang_col:
        lang = st.radio(
            tr("FR", "language_ui"),
            options=["FR", "EN"],
    ...
    if lang == "EN":
        st.markdown(f"<div class='page-title' style='display:none'>{tr(lang, 'title')}</div>", unsafe_allow_html=True)
        st.markdown("<script>document.title = 'Sleep Quality';</script>", unsafe_allow_html=True)
        title_col.empty()
        with title_col:
            st.markdown(f"<div class='page-title'>{tr(lang, 'title')}</div>", unsafe_allow_html=True)
    ...
    left, right = st.columns([1.34, 0.66], gap="large")
    ...
    with right:
        ...
        st.markdown(
            f"<div class='quality-badge' style='background:{badge_color}'><span class='quality-badge-dot'></span>{badge_text}</div>",
            unsafe_allow_html=True,
        )
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        f"<div class='section-card'><div class='section-title' style='font-size:1.8rem'>{tr(lang, 'why')}</div><div class='small-note'>{tr(lang, 'why_caption')}</div>",
        unsafe_allow_html=True,
    )
